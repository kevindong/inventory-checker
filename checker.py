...
if raw_pages is None:
    print(
        'Did not find environmental variable called "WEBPAGES"; will not check anything.'
    )
    sys.exit(1)
ifttt_api_key = os.getenv("IFTTT_API_KEY")
if ifttt_api_key is None:
    print(
        'Did not find environmental variable called "IFTTT_API_KEY" will not check anything.'
    )
    sys.exit(1)
...
for page in pages:
    ...
    try:
        ...
        continue
    if page["text"] not in driver.find_element_by_tag_name("html").get_attribute(
        "innerHTML"
    ):
        requests.post(
            f"https://maker.ifttt.com/trigger/item_available/with/key/{ifttt_api_key}",
            {"value1": page["item_name"], "value2": page["webpage"]},
        )
...
if not something_available:
    requests.post(
        f"https://maker.ifttt.com/trigger/nothing_available/with/key/{ifttt_api_key}",
        {"value1": now, "value2": ",".join(unloaded_pages)},
    )
